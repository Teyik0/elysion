...
>
> 💡 **Furin note** — `defer()` is optimized for SSR. SSG and ISR routes support it, but deferred Promises resolve synchronously during the render pass (no streaming benefit).
