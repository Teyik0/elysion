...
Import `useLogger` from `@teyik0/furin` — not from `evlog/elysia` directly. The Furin wrapper falls back to a no-op logger during background renders (ISR revalidation, SSG pre-render) instead of throwing:

```ts
// src/lib/posts.ts
import { useLogger } from "@teyik0/furin"
...
You can call it directly in a loader too:
...
During ISR background revalidation, the loader **is** re-executed with a synthetic context (no cookies, no headers, empty query). `useLogger()` from `@teyik0/furin` returns a no-op in that case — fields are silently dropped rather than crashing the revalidation.
