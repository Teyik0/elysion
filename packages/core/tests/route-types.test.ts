...
describe("writeRouteTypes", () => {
  ...
  test("creates routes.d.ts with the correct module declaration", () => {
    writeRouteTypes(routes(["/"]), tmpDir);

    const content = readFileSync(join(tmpDir, "routes.d.ts"), "utf8");
    expect(content).toContain("Auto-generated by Elyra");
    expect(content).toContain('import "elyra/link"');
    expect(content).toContain('declare module "elyra/link"');
  ...
  test("idempotent — calling twice with the same routes produces identical output", () => {
    const path = join(tmpDir, "routes.d.ts");
  ...
  test("creates the file even if it did not exist before", () => {
    ...
    expect(existsSync(join(freshDir, "routes.d.ts"))).toBe(false);
    writeRouteTypes(routes(["/"]), freshDir);
    expect(existsSync(join(freshDir, "routes.d.ts"))).toBe(true);
  });
});
